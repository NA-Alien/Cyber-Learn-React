    ...
    <nav className="sticky top-0 z-50 w-full bg-white/90 backdrop-blur-md border-b border-slate-100 px-6 py-4">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        <Link to="/" className="flex items-center gap-2 group">
          ...
          <span className="text-xl font-black tracking-tighter text-slate-900 uppercase">AI Learn</span>
        </Link>
          ...
          {navLinks.map((link) => (
            <Link
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm transition-all ${
                isActive(link.path) ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-900'
              }`}
            >
        ...
        <div className="hidden md:block">
          <button className="px-4 py-2 text-[10px] font-black uppercase tracking-widest text-slate-400 border-2 border-slate-100 rounded-full hover:border-blue-600 hover:text-blue-600 transition-all">Resources</button>
        </div>
      </div>
    </nav>
  );
